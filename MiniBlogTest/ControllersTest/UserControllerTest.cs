namespace MiniBlogTest.ControllerTest
{
    ...
    public class UserControllerTest
    {
        ...
        public async Task Should_register_user_fail_when_UserStore_unavailable()
        {
            ...
            var client = factory.WithWebHostBuilder(builder => { builder.ConfigureServices(services => services.AddSingleton(ServiceProvider => userStoreMocker.Object)); }).CreateClient();
            var userName = "Tom";
            var email = "a@b.com";
            var user = new User(userName, email);
            var userJson = JsonConvert.SerializeObject(user);
